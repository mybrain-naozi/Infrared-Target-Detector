...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(description="Detect all images in others folder.")
...
def main() -> None:
    ...
    build_contact_sheet(preview_items, preview_path, "Others set preview", cols=3)

    summary_lines = [
        "# 检测简报",
        "",
        f"- 输入：`{input_dir.name}`，共 `{len(images)}` 张，成功处理 `{processed}` 张",
        f"- 结果：失败 `{len(failed)}` 张，平均每张 `{(sum(pred_counts) / len(pred_counts)) if pred_counts else 0.0:.2f}` 个框",
        f"- 输出：预览图 `preview.png`，其余单张结果都在当前目录",
        f"- 参数：自动海天线 `{cfg.auto_horizon}`，搜索带高 `{cfg.horizon_band_height}` px，最多保留 `{cfg.max_boxes}` 个框",
    ]
    if failed:
        summary_lines.extend(["", "## 读取失败", ""])
    ...
    print(f"Images:  {processed}/{len(images)} processed")
